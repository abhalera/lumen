...
window.SIMS.cartesian = (function(){
  ...
  function mount(){
    ...
    document.getElementById("preset-bar").innerHTML =
      '<button class="preset-btn active" id="p-c">Colours × objects (Fig 2.1)</button>' +
      '<button class="preset-btn" id="p-p">DL/MP/KA plates (Fig 2.2)</button>' +
      '<button class="preset-btn" id="p-r">R × R plane</button>';
...
window.SIMS.arrows = (function(){
  ...
  function mount(){
    ...
    document.getElementById("preset-bar").innerHTML =
      '<button class="preset-btn active" id="p-27">Fig 2.7  y=x−2</button>' +
      '<button class="preset-btn" id="p-3x">Ex 2.2.1  y=3x</button>' +
      '<button class="preset-btn" id="p-all">All 6 arrows (2⁶=64)</button>';
  ...
  function draw(t){
    ...
    if(mode==="fig27"){
      ...
      for(var k=0;k<show;k++){
        var a=pairs[k];
        m += '<line x1="196" y1="'+(70+a[0]*70)+'" x2="524" y2="'+(70+a[1]*70)+'" stroke="#34d399" stroke-width="2" marker-end="url(#arr)"/>';
      }
...
window.SIMS.funmachine = (function(){
  ...
  function mount(){
    ...
    document.getElementById("preset-bar").innerHTML =
      '<button class="preset-btn active" id="p-ok">Function (Ex 2.3.1 i)</button>' +
      '<button class="preset-btn" id="p-two">Two images (Ex 2.3.1 iii)</button>' +
      '<button class="preset-btn" id="p-miss">Missing image (Example 7)</button>';
...
window.SIMS.catalogue = (function(){
  ...
  function mount(){
    ...
    document.getElementById("preset-bar").innerHTML =
      '<button class="preset-btn active" id="p-abs">|x| Fig 2.13</button>' +
      '<button class="preset-btn" id="p-sgn">signum Fig 2.14</button>' +
      '<button class="preset-btn" id="p-fl">[x] Fig 2.15</button>' +
      '<button class="preset-btn" id="p-circ">√(9−x²) Ex 2.3.2</button>';
  ...
  function draw(t){
    ...
    svg.innerHTML=m;
    readout(cell("x", x.toFixed(2)) + cell("f(x)", isNaN(yv)?"undefined":yv.toFixed(2), "#34d399"));
...
window.SIMS.algebrafn = (function(){
  ...
  function mount(){
    ...
    document.getElementById("preset-bar").innerHTML =
      '<button class="preset-btn active" id="p-s">f + g = 3x−2</button>' +
      '<button class="preset-btn" id="p-d">f − g = −x+4</button>' +
      '<button class="preset-btn" id="p-q">f / g  hole at 3/2</button>';
  ...
  function draw(t){
    ...
    function path(fn, col){
      var d="";
      for(var u=-4;u<=4.05;u+=0.1){
        var yu=fn(u);
        if(mode==="quot" && Math.abs(2*u-3)<0.08){ d+="M "+X(u+0.12)+" "+Y(fn(u+0.12)); continue; }
        if(Math.abs(yu)>8) continue;
        d += (d===""?"M":"L")+" "+X(u)+" "+Y(yu);
      }
    ...
    m += path(comb,"#34d399");
    if(!(mode==="quot" && Math.abs(g(x))<1e-6)) m += '<circle cx="'+X(x)+'" cy="'+Y(comb(x))+'" r="5" fill="#f8fafc"/>';
    if(mode==="quot") m += '<line x1="'+X(1.5)+'" y1="20" x2="'+X(1.5)+'" y2="280" stroke="#f87171" stroke-dasharray="4 4"/>';
    svg.innerHTML=m;
    var val = (mode==="quot" && Math.abs(g(x))<1e-6) ? "undefined" : comb(x).toFixed(2);
...
window.SIMS.domainlab = (function(){
  ...
  function mount(){
    ...
    document.getElementById("preset-bar").innerHTML =
      '<button class="preset-btn active" id="p-sq">√(x−1)</button>' +
      '<button class="preset-btn" id="p-ab">|x−1|</button>' +
      '<button class="preset-btn" id="p-rat">x²/(1+x²) → [0,1)</button>';
  ...
  function Y(y){ return 220 - y*80; }
  function draw(t){
    var svg=svgEl(); if(!svg) return;
    var x=-1+((t||0)/6)*6;
    var m='<rect width="720" height="300" fill="#09131d"/>';
    m += '<line x1="40" y1="220" x2="680" y2="220" stroke="#334155"/><line x1="200" y1="20" x2="200" y2="280" stroke="#334155"/>';
    ...
    svg.innerHTML=m;
    readout(cell("x", x.toFixed(2)) + cell("f(x)", isNaN(yv)?"undefined":yv.toFixed(3), "#34d399") + cell("Domain", dom) + cell("Range", rng));
